...
Bounded inference candidate-generation work (public `resource_limit` on exhaustion),
...
### Phase 1 — Freeze the core language contract ✅ COMPLETE — 2026-08-09 → archived in PLAN-ARCHIVE.md

The normative `core-0.1` reference (`docs/core-language.md`), the 26-case conformance corpus,
and the exact meaning of every public result.
...
The `.tfl` loader, the installable `tfl` command, `tfl-cli-0.1`, and outcome exit statuses.
...
Half-open Unicode code-point spans (`Tfl.Source`) and the failure taxonomy shared across
notation, runtime, source files, and the verifier. Establishes the shared span contract that
Phase 17 declarations must use.
