...
let grade gold got =
  Bench.Score.grade_name (Bench.Score.grade_against [ p gold ] (p got))

let expect name gold got want =
  test name (fun () -> check_eq (grade gold got) want)

(* ── Must match: naming is arbitrary ───────────────────────────────────── *)

let () =
  expect "identical formulas are exact" "-Trustee+Fiduciary"
    "-Trustee+Fiduciary" "exact";
  expect "different term names, same shape" "-Trustee+Fiduciary"
    "-Trustee2+Fiduciary9" "structural";
  (* The case from the 4.3 smoke: models abbreviate verbs. *)
  expect "an abbreviated relation name still matches"
    "+Contractor+(Work+Subsidiary)" "+Contractor+(Wrk+Subsidiary)" "structural";
  expect "a lengthened relation name still matches"
    "+Contractor+(Work+Subsidiary)" "+Contractor+(Work_for+Subsidiary)"
    "structural";
...
let () =
  ...
  expect "an A-form converse is NOT a match" "-Trustee+Fiduciary"
    "-Fiduciary+Trustee" "wrong";
  ...
  expect "unrelated terms do not match" "-Trustee+Fiduciary" "-Penguin+Aircraft"
    "wrong";
  ...
  expect "a singular is not a plain term" "+-Maria*+Trustee" "+Maria+Trustee"
    "wrong";
  expect "a negative term is not a plain one" "-(-Resident)+Taxpayer"
    "-Resident+Taxpayer" "wrong"
...
let () =
  ...
  test "correctness counts exact, structural and equivalent" (fun () ->
      List.iter
        (fun g ->
          check (Bench.Score.counts_as_correct g) "should count as correct")
        [ Bench.Score.Exact; Bench.Score.Structural; Bench.Score.Equivalent ];
      List.iter
        (fun g ->
          check
            (not (Bench.Score.counts_as_correct g))
            "should not count as correct")
...
let () =
  test "an also_ok reading scores on its own merits" (fun () ->
      let accepted =
        [
          p "-(+Temporary+Contractor)-Fiduciary";
          p "-\"temporary contractor\"-Fiduciary";
        ]
      in
      let got = p "-\"temporary contractor\"-Fiduciary" in
      check_eq
        (Bench.Score.grade_name (Bench.Score.grade_against accepted got))
        "exact")
